...
export async function scanRepository(root: string): Promise<RepositoryBrief> {
  ...
  const commands = detectCommands(packageManager, packageJson);
  ...
  return {
    ...
    commands,
    readinessNotes: detectReadinessNotes(root, files, packageManager, readme, commands),
